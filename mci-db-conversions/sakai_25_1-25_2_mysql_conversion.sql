-- done in sakai_23_4-23_5_mysql_conversion.sql
-- START SAK-51950
...
-- START SAK-51938
ALTER TABLE lesson_builder_pages
    ADD hiddenFromNavigation BIT DEFAULT 0;
-- END SAK-51938

-- START SAK-52495
ALTER TABLE GB_GRADABLE_OBJECT_T
    ADD LINEITEM_METADATA LONGTEXT NULL;
-- END SAK-52495
...
ALTER TABLE lti_tool_site
    ADD deployment_group VARCHAR(128) DEFAULT NULL;
-- END SAK-52559

-- START SAK-52583
ALTER TABLE lti_tools
    ADD allowgradebookreadonly TINYINT DEFAULT 0;
-- END SAK-52583
...
DELETE
FROM MFR_PERMISSION_LEVEL_T
WHERE ID NOT IN (SELECT PERMISSION_LEVEL
...
INSERT
IGNORE INTO SAKAI_REALM_FUNCTION (FUNCTION_NAME) VALUES ('assessment.all.groups');
...
VALUES ((select REALM_KEY from SAKAI_REALM where REALM_ID = '!site.template'),
        (select ROLE_KEY from SAKAI_REALM_ROLE where ROLE_NAME = 'maintain'),
        (select FUNCTION_KEY from SAKAI_REALM_FUNCTION where FUNCTION_NAME = 'assessment.all.groups'));
INSERT INTO SAKAI_REALM_RL_FN
VALUES ((select REALM_KEY from SAKAI_REALM where REALM_ID = '!site.template.course'),
...
values ('maintain', 'assessment.all.groups');
INSERT INTO PERMISSIONS_SRC_TEMP
values ('Instructor', 'assessment.all.groups');
...
FROM (SELECT DISTINCT SRRF.REALM_KEY, SRRF.ROLE_KEY FROM SAKAI_REALM_RL_FN SRRF) SRRFD
...
VALUES ('sakai_25_1-25_2_mysql_conversion.sql', NOW(6));